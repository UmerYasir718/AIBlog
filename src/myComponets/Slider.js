...
export default function Slider({ className }) {
    ...
    const userData = async () => {
        try {
            const uData = await fetch(`https://aiblog-mu.vercel.app/user?email=${user.email}`, {
                method: "POST",
    ...
    const checkOutBtn = async () => {
        ...
        const response = await fetch(`https://aiblog-mu.vercel.app/api/create-checkout-session?email=${user.email}`, {
            method: "POST",
            headers: headers,
    ...
    return (
        <div>
            <div className="d-flex flex-column flex-shrink-0 p-3 Slider">
                <Link to="/" className="logoApp d-flex align-items-center mb-3 mb-md-0 me-md-auto mx-auto">
                ...
                <button className="newchatbtn mb-2" onClick={checkOutBtn}>
                    <i class="fa-solid fa-coins" style={{ color: "#e6d40a" }}></i> {cerdit ? cerdit.token : 0}
                </button>

...
export default function Slider({ className }) {
    ...
    const fetchData = async () => {
        try {
            const response = await fetch(`http://localhost:8000/post?email=${user.email}`);
            if (!response.ok) {
    ...
    const userDataDb = async () => {
        try {
            const datadb = await fetch(`http://localhost:8000/userdata?email=${user.email}`);
            if (!datadb.ok) {
                throw new Error('Failed to fetch data');
            }
    ...
    const userData = async () => {
        try {
            const uData = await fetch(`http://localhost:8000/user?email=${user.email}`, {
                method: "POST",
    ...
    const checkOutBtn = async () => {
        const stripe = await loadStripe("pk_test_51NnIorCqPnESIcUCUezx9Ae1Tfcxrnhqaxi0KYluXByG3dEuMUMlLY8FcJG3eUzajEWLL8oyS7OJuzUgMKcOpGlH00QwR1IcOn");
        ...
        const response = await fetch(`http://localhost:8000/api/create-checkout-session?email=${user.email}`, {
            method: "POST",
            headers: headers,
    ...
    return (
        <>
            ...
            <Offcanvas className="d-flex flex-column flex-shrink-0 p-3 Slider" show={show} onHide={handleClose}>
                <Offcanvas.Header closeButton>
                    <Offcanvas.Title>
                        <Link to="/" className="logoApp d-flex align-items-center mb-md-0 me-md-auto mx-auto">
                ...
                <Offcanvas.Body>
                    ...
                    <button className="newchatbtn mb-2" onClick={checkOutBtn}>
                        {cerdit ? cerdit.token : 0}  <i class="fa-solid fa-coins" style={{ color: "#e6d40a" }}></i>
                    </button>
